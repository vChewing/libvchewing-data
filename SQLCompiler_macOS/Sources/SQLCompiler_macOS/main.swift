  ...
  var taskFlags: TaskFlags = [.common, .chs, .cht] {
    didSet {
      ...
      if compileSQLite {
        var succeeded = true
        writeDB: do {
          try DBDataWriter.writeOnce(to: URL(fileURLWithPath: urlSQLite)) {
            NSLog("// 開始整合反查資料。")
            ...
            NSLog("// 反查資料整合完畢。")
            NSLog("// 準備建置 SQL 資料庫。")
            var allModels = [any PersistentModel]()
            allModels.append(contentsOf: rangeMapReverseLookup.toRevModels())
            allModels.append(contentsOf: rangeMapJSONCHS.toMainModels(.chs))
            allModels.append(contentsOf: rangeMapJSONCHT.toMainModels(.cht))
            allModels.append(contentsOf: rangeMapSymbols.toMainModels(.symb))
            allModels.append(contentsOf: rangeMapZhuyinwen.toMainModels(.chew))
            allModels.append(contentsOf: rangeMapCNS.toMainModels(.cns))
            return allModels
          }
        } catch {
